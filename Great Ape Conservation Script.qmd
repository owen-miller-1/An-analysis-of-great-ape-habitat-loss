...
format:
  html:
    ...
    include-in-header:
      text: |
        <!-- Google tag (gtag.js) -->
        <script async src="https://www.googletagmanager.com/gtag/js?id=G-CZDTEZ3V3Y"></script>
        <script>
          window.dataLayer = window.dataLayer || [];
          function gtag(){dataLayer.push(arguments);}
          gtag('js', new Date());
          gtag('config', 'G-CZDTEZ3V3Y');
        </script>
...
*This analysis examines 25 years of satellite-derived tree cover data within the ranges of all non-human great apes: chimpanzees, gorillas, bonobos, and orangutans. Between 2001 and 2025, core great ape range countries lost approximately 84 million hectares of tree cover, equivalent to 11.8% of their combined tree cover extent in 2000. The rate of forest loss is substantially lower in protected areas, by roughly 63% in Africa and 73% in Asia. Yet only about a fifth of mapped great ape range falls within protected areas. The data suggest that protection helps, but far too little habitat is protected, and loss continues even within protected boundaries. Without urgent action, the forests these animals depend on may disappear within our lifetime. And with them, our closest relatives on Earth.*      


## Overview

Great apes, our closest living relatives, are facing extinction. All non-human great apes, gorillas, chimpanzees, bonobos, and orangutans, are classified as either Endangered (EN) or Critically Endangered (CR) by the International Union for Conservation of Nature (IUCN). Without urgent action, there is a real possibility that current generations of humans will be the last to observe these animals in the wild. 

The figure below displays the IUCN's conservation status for each of the four great apes. Select a card to view population estimates of the species and subspecies in each group. Estimates are drawn from IUCN Red List assessments (2017-2024). See [Sources](#sources) for full citations.  
...
Even when considering the upper bounds of these estimates, the total population of great apes remaining on Earth is less than a million individuals. Some species, the bornean orangutan in particular, have declined by at least 60% between 1950 and 2010, likely surpassing 80% by 2025 [@ancrenaz2024]. Great apes face a range of human-driven threats, including poaching, civil conflict, disease transmission, and the illegal wildlife trade, but it is largely agreed that the most pervasive threat to their existence in the wild is the destruction of their forest habitat.     

## Methodology

### Scope
This analysis covers 13 countries representing the core great ape range across equatorial Africa and Southeast Asia: Cameroon, Central African Republic, Democratic Republic of the Congo, Equatorial Guinea, Gabon, Nigeria, Republic of the Congo, Rwanda, Burundi, Tanzania, Uganda, Indonesia, and Malaysia. Full native chimpanzee range extends further into West Africa, home to the Western Chimpanzee, and a small, poorly surveyed area in South Sudan; neither is represented here. 

### Data sources
Forest cover data is sourced from the Hansen Global Forest Change (GFC) dataset (GFC-2025-v1.13) [@hansen2013]. In this analysis, forest is defined as areas with tree canopy cover greater than 10%, following the Food and Agriculture Organization (FAO) definition. Protected area boundaries are sourced from the World Database on Protected Areas (WDPA) [@unepwcmc2026], and great ape range polygons from the IUCN Red List of Threatened Species spatial data. Methodological detail, including the rationale for the canopy threshold and the distinction between mapped range and true habitat, is available in [Methodological notes](#methodological-notes) below. See [Sources](#sources) for full citations. 

### Approach
The protected area effectiveness analysis clips Hansen raster data to mapped great ape range polygons and classifies pixels as inside or outside protected areas using WDPA boundaries. Cumulative and annual forest loss rates (as a percentage of 2000 forest extent) are then compared between the two groups.

## Habitat loss

Most gorillas, chimpanzees and bonobos live in tropical rainforests across equatorial Africa, while orangutans are native to the forests of Southeast Asia, forests that are disappearing at an alarming rate due to human activity. The chart below illustrates the annual percentage and cumulative tree cover loss in core great ape range countries from 2001 to 2025. Select the tabs to toggle between the two views. Annual loss is expressed as a percentage of each country's forest extent in 2000; cumulative loss is shown in total hectares. 
...
# Load data
df = pd.read_csv('data/Forest_data.csv', encoding='latin-1')

ape_countries = [
    'Democratic Republic of the Congo', 'Cameroon', 'Gabon',
    'Republic of the Congo', 'Central African Republic', 'Nigeria',
    'Uganda', 'Rwanda', 'Burundi', 'Tanzania', 'Equatorial Guinea',
    'Indonesia', 'Malaysia'
]

# Shortened longer country names
country_names = {
...
Many protected areas within great ape ranges are listed as 'Not Applicable' or 'Not Reported' in the World Database on Protected Areas (WDPA), maintained by UNEP-WCMC and IUCN, indicating incomplete reporting rather than absence of protection. For example, Volcanoes National Park in Rwanda, a critical mountain gorilla sanctuary, is listed without an IUCN category despite being a well-managed Category II national park.

The figure below shows protected areas overlaid with great ape ranges. Unlike the country-level figures above, the analysis that follows is confined *specifically* to land within these ranges, examining whether protection status is associated with reduced forest loss. The Africa map shows gorilla, chimpanzee, and bonobo ranges as separate layers, with the two gorilla species combined; the Asia map shows the three orangutan species individually, given their distinct ranges. 
...
ape_africa = ape_africa.clip(core_bbox)

pa_africa = protected_areas[protected_areas['sci_name'].isin(africa_species)].copy()
pa_africa = pa_africa.clip(core_bbox)
...
fig_coverage_pct.update_layout(
    title=dict(
        text='<b>Great ape range within protected areas, by country</b><br>'
             '<sup><i>Percentage of mapped ape range falling within a protected area boundary</i></sup>',
        x=0.02
    ),
    template='simple_white',
    font_family='Roboto',
...
|:---------------------|------------------:|--------------------:|--------------:|
| Nigeria              |              3.06 |                1.71 |          56.1 |
| Rwanda               |              0.26 |                0.12 |          48.2 |
| Rep. of Congo        |             24.67 |               10.06 |          40.8 |
| Uganda               |              2.03 |                0.82 |          40.6 |
| Tanzania             |              1.66 |                0.40 |          24.1 |
| Gabon                |             25.47 |                5.30 |          20.8 |
| Malaysia             |              4.92 |                0.93 |          18.9 |
| DR Congo             |            123.03 |               21.09 |          17.1 |
| Equatorial Guinea    |              2.46 |                0.42 |          16.9 |
| Indonesia            |             23.56 |                3.57 |          15.2 |
| Central African Rep. |             12.78 |                1.84 |          14.4 |
| Cameroon             |             23.49 |                2.85 |          12.1 |
| Burundi              |              0.62 |                0.05 |           8.2 |
| **Total**            |        **247.99** |           **49.16** |      **19.8** |
...
Notably, percentage coverage does not always track absolute protection. DR Congo has the largest area protected in this analysis by far, but it still represents just 17.1% of total great ape range, demonstrating that raw percentages can understate the scale of protection in large-range countries. 
...
ape_countries = ['Cameroon', 'Central African Republic', 'Democratic Republic of the Congo',
                  ...
                  'Tanzania', 'Uganda', 'Equatorial Guinea', 'Indonesia', 'Malaysia']

df = pd.read_csv('data/pa_effectiveness_by_country_combined.csv')
...
africa_countries = ['Cameroon', 'Central African Republic', 'Democratic Republic of the Congo',
                     'Gabon', 'Nigeria', 'Republic of the Congo', 'Rwanda', 'Burundi', 'Equatorial Guinea',
                     'Tanzania', 'Uganda']
...
fig_line_pa.update_layout(
    ...
    xaxis=dict(
        ...
        tickmode='array',
        tickvals=[2000, 2005, 2010, 2015, 2020, 2025],
        showgrid=False
    ),
    hoverlabel=dict(bgcolor='white', bordercolor='#333333', font_size=13, align='left'),
    hovermode='x unified',
...
Republic of Congo offers a useful comparison. It has a much larger ape range, but a similar protection percentage. Here, forest loss inside protected areas was only about 2 percentage points lower than outside over the full period (a 54.5% relative reduction), a far smaller gap than Rwanda's. Concerningly, by 2024 and 2025, that gap had nearly disappeared, with loss rates inside and outside protected areas converging to nearly identical levels.

These comparisons show that forest loss could be correlated with protection status, but not that protection necessarily *caused* the difference. Protected areas are rarely placed at random. Governments tend to protect land that is steep, remote, far from populated centres, and poorly suited to agriculture or other development. In other words, the land least likely to have been cleared in any case. Some portion of the gap observed here therefore reflects where protected areas were placed rather than what protection accomplished, and the true effect of protection is likely smaller than what these raw differences suggest. Quantifying the effect of protection holistically would require matching protected pixels to unprotected ones with comparable characteristics (human population density, steepness, fertility, proximity to roads, etc.), then comparing loss within matched pairs [@joppa2009]. That analysis is beyond the scope of this piece, but it is the natural next step. The figures here should therefore be interpreted as an *upper bound* of protection's effect rather than an estimate of it.

## Conclusion
It is difficult to overstate the seriousness of the situation facing great apes in the wild today. Orangutans, gorillas, chimpanzees and bonobos are on the brink of extinction. The countries they inhabit lost a combined 84 million hectares of forest cover in just the past 25 years, equivalent to 11.8% of forest extent in 2000. The cause of forest loss varies by country, but the primary drivers are agricultural conversion and logging. 

Governments have established protected areas, partly to slow this concerning trend. This analysis found that forest loss is substantially lower within protected areas in great ape ranges: 63% less in African countries and 73% in Asian countries. While this is an encouraging result, only about a fifth (19.8%) of mapped ape ranges fall within protected areas. Effectiveness also varies greatly by country. In Malaysia, the rate of forest loss was 94% less within protected great ape ranges, a striking result that could support the case for further establishment and enforcement of PAs in the region. Conversely, the rate of loss in countries like the DR Congo and Republic of Congo was closer to 55-60% lower within PAs than outside of them. These are still meaningful results, but a noticeably smaller effect. Protection also appears to be more effective against deliberate clearing and logging than against unpredictable climate events, as Indonesia's 2015–16 fire season illustrated, when loss inside protected areas briefly exceeded loss outside them entirely. 
...
- **Double counting issues**: During the analysis, two double counting issues were identified. The WDPA is an aggregation of many different sources, which sometimes result in overlap of protected area boundaries and duplicates for the same site. For example, Odzala-Kokoua National Park in the Republic of Congo has three separate WDPA records (national park, UNESCO Biosphere Reserve, and Ramsar wetland designations) which all appear twice in the source data. The original pipeline summed all protected area polygons independently, resulting in an inflated total in countries with duplicate or overlapping records. A dissolve was added to merge overlapping boundaries before calculating area, and the coverage figures in the [Protected areas](#protected-areas) section reflect the corrected calculation. The other double counting issue was identified in the [Does protection reduce forest loss?](#does-protection-reduce-forest-loss) pipeline. This occurred in areas with overlapping great ape range polygons. For example, Nigeria has some areas where gorillas live alongside chimpanzees. The original process summed both ranges independently, which inflated the total ape range in countries with overlapping ranges. A dissolve was added here as well to merge ranges, and the results presented here represent the corrected calculation. 
</details>
